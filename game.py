...
for i in range(1, 6):
    protagonist_walk_sprites.append(pygame.image.load('img/protagonist_walk_' + str(i) + '.png'))
background = pygame.image.load('img/background.png')
block_sprites = []
for i in range(0, 12):
    block_sprites.append(pygame.image.load("img/block_" + str(i) + ".png"))
skeleton_sprites = []
for i in range(1, 5):
    skeleton_sprites.append(pygame.image.load('img/skeleton' + str(i) + ".png"))
skeleton_sprites.append(pygame.image.load('img/skeleton_attack_left.png'))
skeleton_sprites.append(pygame.image.load('img/skeleton_attack_right.png'))
selected_block = pygame.image.load('img/selected_block.png')
wooden_sword_sprites = []
wooden_sword_sprites.append(pygame.image.load('img/wooden_sword_right.png'))
wooden_sword_sprites.append(pygame.image.load('img/wooden_sword_left.png'))
wooden_pickaxe_sprites = []
wooden_pickaxe_sprites.append(pygame.image.load('img/wooden_pickaxe_right.png'))
wooden_pickaxe_sprites.append(pygame.image.load('img/wooden_pickaxe_left.png'))
inventory_slot = pygame.image.load('img/inventory_slot.png')
highlighted_inventory_slot = pygame.image.load('img/highlighted_inventory_slot.png')
arrow_left = pygame.image.load('img/arrow_left.png')
arrow_right = pygame.image.load('img/arrow_right.png')
wooden_bow_sprites = []
wooden_bow_sprites.append(pygame.image.load('img/wooden_bow_left.png'))
wooden_bow_sprites.append(pygame.image.load('img/wooden_bow_right.png'))
dynamite_monster_sprites = []
dynamite_monster_sprites.append(pygame.image.load('img/dynamite_monster.png'))
explosion = pygame.image.load('img/explosion.png')
lava_monster_sprites = []
lava_monster_sprites.append(pygame.image.load('img/lava_monster.png'))
...
class World():
    ...
    def spawn_monster(self):
        if random.random() < (0.0025 * (1 / 2) ** self.monster_count) and self.monster_count <= 10:
            ...
            if able_to_spawn:
                ...
                print('Skeleton spawned at (%d, %d)' % (spawn_area.x, spawn_area.y))
        if random.random() < 0.0005 * (1 / 2) ** self.monster_count and self.monster_count <= 10:
            ...
            if able_to_spawn:
                ...
                self.monster_count += 1
                print('Dynamite Monster spawned at (%d, %d)' % (spawn_area.x, spawn_area.y))
        if random.random() < 0.0005 * (1 / 2) ** self.monster_count and self.monster_count <= 10:
            spawn_area = pygame.rect.Rect(random.randint(-600, -300) + self.player.x if random.random() < 0.5 \
                                              else random.randint(300, 600) + self.player.x, \
                                          self.player.y - random.randint(150, 210), 100, 100)
            ...
            if able_to_spawn:
                ...
                self.monster_count += 1
                print('Lava monster spawned at (%d, %d)' % (spawn_area.x, spawn_area.y))

    def despawn_monster(self):
        for entity in self.entities:
            if math.sqrt((entity.x - self.player.x) ** 2 + (entity.y - self.player.y) ** 2) > 2000:
                self.entities.remove(entity)
                print('Entity removed at (%d, %d)' % (entity.x, entity.y))
                self.monster_count -= 1
    ...
    def physical_engine(self):
        ...
        future_hitbox_x = pygame.rect.Rect((self.player.x + self.player.vx, self.player.y),
                                           (self.player.width, self.player.height))
        future_hitbox_y = pygame.rect.Rect((self.player.x, self.player.y + self.player.vy),
                                           (self.player.width, self.player.height))
    ...
    def render_frame(self):
        ...
        for row in self.current_chunk:
            ...
            for block in row:
                if self.camera_x - block.width <= block.x <= self.camera_x + GAME_FRAME_WIDTH \
                    ...
                    screen.blit(block.sprite,
                                (block.x - self.camera_x, block.y - self.camera_y, block.width, block.height))
        ...
        for entity in self.entities:
            ...
            if self.camera_x - entity.width <= entity.x <= self.camera_x + GAME_FRAME_WIDTH \
                ...
                if isinstance(entity, ItemEntity) and entity.item.owner.hp <= 0:
                    continue
                screen.blit(entity.sprite,
                            (entity.x - self.camera_x, entity.y - self.camera_y, entity.width, entity.height))
                if isinstance(entity, Enemy):
                    health_percentage = entity.hp / entity.max_hp
                    pygame.draw.rect(screen, ((1 - health_percentage) * 255, health_percentage * 255, 60),
                                     pygame.rect.Rect( \
                                         entity.x - self.camera_x - 22, entity.y - self.camera_y - 35,
                                         100 * health_percentage, 15))
        ...
        if self.player.invincibility_frame % 2 == 0:
            ...
            GAME_FRAME_WIDTH // 2 - self.player.width // 2, GAME_FRAME_HEIGHT // 2 - self.player.height // 2))
        pygame.draw.rect(screen, (255, 0, 0),
                         pygame.rect.Rect(GAME_FRAME_WIDTH - 200, 0, (self.player.hp / self.player.max_hp) * 200, 15))
        if self.chosen_block is not None and self.player.inventory.current_item == 1:
            pygame.draw.rect(screen, (255, 255, 0), \
                             pygame.rect.Rect(self.player.get_center_x() - self.camera_x - 50,
                                              self.player.y - self.camera_y - 25,
                                              100 - (self.chosen_block.hp / self.chosen_block.max_hp) * 100, 15))
        # Draws the inventory
        ...
        screen.blit(writing_font.render("Your score: " + str(self.player.score), True, [255, 0, 0]),
                    (GAME_FRAME_WIDTH // 2 - 40, 0, 75, 25))
        pygame.display.flip()
    ...
    def iterate(self):
        ...
        if world.player.hitbox.colliderect(self.hitbox) and world.player.invincibility_frame <= 0:
            ...
            if world.player.hp < 0:
                world.player.hp = 0
        for entity in world.entities:
            if entity.hitbox.colliderect(self.hitbox) and not isinstance(entity, DynamiteMonster) and not isinstance(
                    entity, Explosion):
                entity.hp -= 20
                if entity.hp < 0:
                    entity.hp = 0
